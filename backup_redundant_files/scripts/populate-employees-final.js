...
/**
 * Script final para poblar la base de datos con 800 empleados
 * Maneja duplicados y genera emails únicos
 */
...
const firstNames = [
  ...
  'Pedro', 'Gabriel', 'Martín', 'Esteban', 'Andrés', 'Felipe', 'Roberto', 'Carlos',
  'Javier', 'Sergio', 'Luis', 'Miguel', 'Alberto', 'Ricardo', 'Eduardo', 'Fernando'
];
...
const lastNames = [
  ...
  'Valdés', 'Fuentes', 'Toro', 'Leiva', 'Araya', 'Rojas', 'Paredes', 'Mendoza',
  'Ortega', 'Carrasco', 'Fuentes', 'Rojas', 'Morales', 'Castillo', 'Vargas'
];
...
// Conjunto para trackear emails únicos
...
function generateUniqueEmail(name, companyName, index) {
  ...
  do {
    // Agregar número aleatorio o índice para garantizar unicidad
    const suffix = attempts === 0 ? '' : `${index}${attempts}`;
    email = `${nameParts[0]}.${nameParts[1]}${suffix}@${companyDomain}.cl`;
    attempts++;
...
async function clearExistingEmployees() {
  try {
    ...
    usedEmails.clear(); // Limpiar el set de emails
    return true;
  } catch (error) {
    logError(`Error al limpiar empleados: ${error.message}`);
    return false;
...
async function generateEmployees() {
  log('🚀 Generando 800 empleados con emails únicos', 'bright');
  log('═'.repeat(60), 'blue');
  ...
  for (let i = 0; i < companies.length; i++) {
    ...
    for (let j = 0; j < employeesForThisCompany; j++) {
      ...
      const employee = {
        ...
        name: fullName,
        email: generateUniqueEmail(fullName, company.name, employeeIndex),
      ...
      try {
        ...
        if (error) {
          logError(`Error al insertar empleado ${employee.name}: ${error.message}`);
        } else {
          totalGenerated++;
          employeeIndex++;
        ...
        logError(`Error al insertar empleado: ${error.message}`);
      }
    }
  }
...
// Función principal
async function main() {
  log('📊 SCRIPT FINAL DE POBLACIÓN DE EMPLEADOS - BrifyRRHH', 'bright');
  log('Este script generará 800 empleados con emails únicos', 'blue');
  log('═'.repeat(60), 'blue');
