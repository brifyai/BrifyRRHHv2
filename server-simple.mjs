...
// Endpoint para manejar el callback de Google OAuth
app.get('/auth/google/callback', async (req, res) => {
  try {
    ...
    console.log('- State:', state || 'No especificado');
    ...
    if (!state) {
      console.error('❌ No se recibió estado de OAuth');
      return res.redirect('http://localhost:3000?error=no_state');
    }

    // Extraer userId y codeVerifier del state
    let userId = null;
    ...
    if (!userId) {
      console.error('❌ UserId no encontrado en state');
      return res.redirect('http://localhost:3000?error=no_user_id');
    }

    console.log(`🔄 Procesando tokens para usuario: ${userId}`);
    ...
    // Guardar credenciales en Supabase usando conexión directa
    console.log('💾 Intentando guardar credenciales directamente...');

    try {
      // Usar conexión directa con service role key para evitar problemas de lazy loading
      ...
      if (error) {
        console.error('❌ Error guardando credenciales directamente:', error);
        return res.redirect('http://localhost:3000?error=save_failed');
      }
