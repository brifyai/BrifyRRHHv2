...
const DashboardInnovador = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      // Cargar empresas y empleados por empresa
      console.log('Dashboard: Loading companies and employee counts')
      try {
        // Obtener todas las empresas usando el servicio de comunicación
        const companiesData = await communicationService.getCompanies();
        console.log('Dashboard: Companies loaded via service:', companiesData)
        ...
        if (companiesData && companiesData.length > 0) {
          ...
          const employeesCountPromises = companiesData.map(async (company) => {
            // Obtener empleados usando el servicio de comunicación
            const employees = await communicationService.getEmployees({ companyId: company.id });
            console.log(`Dashboard: Employees for ${company.name}:`, employees.length)
          ...
          console.log('Dashboard: Employee counts data:', employeesCountData)
          setEmployeesByCompany(employeesCountData)
          console.log('Dashboard: Employee counts loaded:', employeesCountData)
        } else {
          console.log('Dashboard: No companies found')
        }
      } catch (companiesError) {
        console.error('Error loading companies:', companiesError)
      }
      
      // Cargar estadísticas de comunicación
      console.log('Dashboard: Loading communication stats')
      try {
        const stats = await communicationService.getCommunicationStats();
        console.log('Dashboard: Communication stats loaded:', stats)
        ...
        console.error('Error loading communication stats:', statsError)
      }
  ...
  // Efecto para depurar el estado de employeesByCompany
  useEffect(() => {
    console.log('Dashboard: employeesByCompany updated:', employeesByCompany)
  }, [employeesByCompany])
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {employeesByCompany && employeesByCompany.length > 0 ? (
                ...
                <div className="text-gray-500 text-center py-4 col-span-full">
                  <p>{!employeesByCompany ? 'Cargando datos de empresas...' : 'No se encontraron empresas'}</p>
                  <p className="text-sm mt-2">
                    Estado: companies={companies?.length || 0}, employeesByCompany={employeesByCompany?.length || 0}
                  </p>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-200">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-blue-50 rounded-xl p-4 text-center">
                ...
                <p className="text-2xl font-bold text-gray-900 mt-2">{communicationStats.totalSent}</p>
                <p className="text-sm text-gray-600 mt-1">Enviados</p>
              </div>
              
              <div className="bg-green-50 rounded-xl p-4 text-center">
                <CheckCircleIcon className="h-8 w-8 text-green-600 mx-auto" />
                <p className="text-2xl font-bold text-gray-900 mt-2">{communicationStats.totalDelivered}</p>
                <p className="text-sm text-gray-600 mt-1">Entregados</p>
              </div>
              
              <div className="bg-purple-50 rounded-xl p-4 text-center">
                <EyeIcon className="h-8 w-8 text-purple-600 mx-auto" />
                <p className="text-2xl font-bold text-gray-900 mt-2">{communicationStats.totalRead}</p>
          ...
          {/* Progreso de almacenamiento y uso de tokens */}
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-200">
            <h2 className="text-xl font-bold text-gray-900 mb-6 flex items-center">
            ...
            <div className="mb-6">
              <div className="flex justify-between text-sm font-medium text-gray-900 mb-2">
                <span className="flex items-center">
                  <CloudIcon className="h-4 w-4 mr-1 text-blue-500" />
                  Almacenamiento
                </span>
                <span>{formatBytes(stats.storageUsed)} de {formatBytes(plans.find(p => p.id === userProfile?.current_plan_id)?.storage_limit_bytes || 1073741824)}</span>
              </div>
              ...
              <p className="text-xs text-gray-500 mt-1 text-right">{getStoragePercentage().toFixed(1)}% usado</p>
            </div>
            ...
            <div>
              <div className="flex justify-between text-sm font-medium text-gray-900 mb-2">
                ...
                <span>{stats.tokensUsed.toLocaleString()} de {stats.tokenLimit.toLocaleString()}</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-3">
                <div 
                  className={`h-3 rounded-full transition-all duration-500 ${
                    (stats.tokensUsed / stats.tokenLimit) * 100 >= 90 ? 'bg-red-500' : 
                    (stats.tokensUsed / stats.tokenLimit) * 100 >= 70 ? 'bg-yellow-500' : 'bg-purple-500'
                  }`}
        ...
        {/* Sección de acciones rápidas y plantillas */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          {/* Acciones rápidas */}
          <div className="bg-white rounded-2xl shadow-xl p-6 border border-gray-200 lg:col-span-1">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Acciones Rápidas</h2>
            <div className="space-y-3">
              <Link
                to="/folders"
                className="flex items-center p-3 rounded-lg border border-gray-200 hover:bg-blue-50 transition-colors duration-200 group"
              >
                <PlusIcon className="h-5 w-5 text-blue-600 mr-3" />
                <span className="text-sm font-medium text-gray-900 group-hover:text-blue-700">Crear Carpeta</span>
              </Link>
              ...
              >
                ...
                <span className="text-sm font-medium text-gray-900 group-hover:text-blue-700">Ver Carpetas</span>
              </Link>
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {userExtensions.map((userExt) => (
                          <div key={userExt.id} className="flex items-center justify-between p-2 bg-blue-50 rounded-lg border border-blue-100">
                            <div className="flex items-center">
...
export default DashboardInnovador
