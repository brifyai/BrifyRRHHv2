  ...
  const searchLaws = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.REACT_APP_SUPABASE_URL}/rest/v1/leyes_con_contenido?select=*&or=(${encodeURIComponent('Título de la Norma')}.ilike.*${encodeURIComponent(searchTerm)}*,${encodeURIComponent('Contenido')}.ilike.*${encodeURIComponent(searchTerm)}*)`,
        {
          headers: {
